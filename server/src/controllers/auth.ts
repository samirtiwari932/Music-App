...
import {sendForgetPasswordLink, sendPassResetSuccessEmail, sendVerificationMail} from "#/utils/mailHelper";
...
export const sendReVerificationMail: RequestHandler = async (req, res) => {
...
export const generateForgetPasswordLink: RequestHandler = async (req, res) => {
  ...
  //generate the link
  //https://yourapp.com/reset_password?token=hfhhdgifhi&userId=fdkjkofjdioj
  await PasswordResetToken.findOneAndDelete({
    owner: user._id,
  });
...
export const updatePassword: RequestHandler = async (req, res) => {
  ...
  if (matched) return res.status(403).json({error: "The new password must be different  !"});
